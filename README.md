...
- **Canonical domain:** set `SITE_URL` in `src/config.ts` (used by the sitemap generator and SEO)
  to the real production domain before launch (currently `https://kytyps5.github.io`).
- **`reportRepoUrl`:** already points at the real emulator repo so forms never 404; switch it to
  this website's repo once published to enable automated report conversion.
