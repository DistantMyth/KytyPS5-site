...
import { COMPAT_REPORTS, STATUSES, STATUS_META, aggregateStatus, computeStats, groupReportsByGame } from "@/lib/compat";
import { Reveal } from "@/components/layout/reveal";

export function CompatPreview() {
  // One game per entry — status is the majority vote across its reports.
  const stats = computeStats(
    [...groupReportsByGame(COMPAT_REPORTS).values()].map((group) => aggregateStatus(group)),
  );
